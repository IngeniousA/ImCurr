/*
	...
	██║╚██████╗██████╔╝███████╗██║  ██╗
	╚═╝ ╚═════╝╚═════╝ ╚══════╝╚═╝  ╚═╝
	Simplified ImCurr, perfectly fit into 32KB file.
	Made by Sergey 'Ingenious' Rakhmanov
	github.com/IngeniousA
*/
#include <cstring>
#include <vector>
#include <stdint.h>
...
class Blowfish {
public:
	Blowfish(const strV &key);
	strV Encrypt(const strV &src) const;
	strV Decrypt(const strV &src) const;
...
namespace {
	...
	size_t PKCS5PaddingLength(const strV &data) {
		if (data.empty())
			return 0;
...
Blowfish::Blowfish(const strV &key) {
...
strV Blowfish::Encrypt(const strV &src) const {
	strV dst = src;
...
strV Blowfish::Decrypt(const strV &src) const {
	strV dst = src;
...
{
	if (argc != 5) {
		printf("%s", "Wrong argument amount");
		return 0;
	}
	if (exists(argv[1]))
		core(argv[1], argv[2], argv[3], argv[argc - 1][0]);
	else
		printf("%s%s", "Cannot get ", argv[1]);
	return 0;
}